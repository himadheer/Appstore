...
class AppStore extends Component {
  ...
  getFilteredApps = () => {
    const {activeTab, intialAppList} = this.state

    const filteredApps = intialAppList.filter(
  ...
  getFilteredApps = searchApps => {
    const {activeTab} = this.state
    const filteredApps = searchApps.filter(each => each.category === activeTab)
        ...
        <div className="app-container">
          ...
          <ul className="tabs-container">
            {tabsList.map(each => (
              <TabItem
                ...
                isActiveTab={activeTab === each.tabId}
              />
            ))}
          </ul>
          ...
          <ul className="apps-container">
            {filteredApps.map(each => (
              <AppItem key={each.appId} apps={each} />
            ))}
          </ul>
        </div>
      </div>
